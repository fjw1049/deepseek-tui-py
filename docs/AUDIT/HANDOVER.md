...
| ⚠️ 4.1.simplified: /prompt 不调 LLM | 4.1 | `AppRuntime.handle_prompt` 现在 emit Rust parity 的 ResponseStart/Delta/End 3-frame 序列 + /prompt/stream SSE 路由（commit `23a5712`），**Rust 本身也没真调 LLM**，只 emit 相同 3 frame。**还差**：真实 Engine/turn_loop 接通（让 delta 携带 LLM streaming token，而不是固定 `"model-selected"`） | 真实 LLM streaming 要等 Engine 和 approval handler 稳定 | 在 `AppRuntime` 里加 Engine + ApprovalHandler + cancel event；`stream_prompt` 把 TurnStartedEvent/AssistantMessageDelta/ToolResultEvent/TurnCompleteEvent 转成 SSE frames；Rust 近期也会往这个方向做 |

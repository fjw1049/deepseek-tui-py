...
| ⬜ 4.1.simplified: /prompt 不调 LLM | 4.1 | `AppRuntime.handle_prompt` 只把 prompt 写入新 thread，不真的发 LLM 请求；`{"output": "accepted on thread_..."}` | 要调 LLM 需要 Engine + approval handler + streaming SSE，Stage 4.1 scope 聚焦路由骨架 | 把 `AppRuntime` 里加 `Engine` 实例（或工厂），/prompt 构造 `SendMessageOp`，把 `TurnStartedEvent/AssistantMessageDelta/TurnCompleteEvent` 转成 SSE 返回客户端。参考 Rust `Runtime::handle_prompt` 的 event collection 模式 |

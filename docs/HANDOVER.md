...
| **累计（历史）** | | | **1323 passed, 4 skipped**（Rust→Python parity 套件；已退役，勿再以此数为门禁） |
| stage8-tui-fix | — | **TUI P0 + Workbench 错误链**：`SendMessageOp` 导入、`/clear` 清 session、`/mode` 同步 `engine.mode`、`readRuntimeError(detail)`、OpenAPI `trust_mode`/compact `reason`、`tests/test_tui_smoke.py` | +2 smoke |
| capability-modules.phase1-7.scoped-bindings-audit | — | **Capability Module Refactor foundation + runtime adapters + PromptContributor split + service-first lookup + scoped bindings audit（2026-06-11）**：新增 `src/deepseek_tui/host/` 宿主基础和 `src/deepseek_tui/capabilities/` 第一方能力适配层；`ToolContext.services` 已接入；公开 `Engine.create()`、`create_tool_runtime()`、`build_default_registry()` 进入 host assembler 后委托兼容实现；`builder.py` 遍历 `default_tool_packs()`；LSP/MCP/Tasks/SubAgents/Automation runtime 构造与 legacy binding 已迁到 capability adapter；`build_system_prompt()` 改为 base prompt + ordered contributors，并将 Skills/Workflow/Memory/Evolution/core prompt blocks 迁到 capability；Memory/Evolution/Goal runtime 构造和 legacy binding 已迁到 capability；Workflow tool-call scoped metadata 迁到 `capabilities/workflow.py`；RLM progress scoped binding 迁到 `capabilities/rlm.py`；SubAgent parent cancel、parent completion sink、loop runtime attachment、per-turn parent cancel refresh 已迁到 `capabilities/subagents.py`。Goal/Memory/Automation/MCP/Evolution 工具优先读 `ToolContext.services` typed/named services，再 fallback 到旧 `metadata`，旧 metadata 仍保留兼容。审计后的剩余高耦合区：PostTurnOrchestrator lifecycle、Memory recall/capture/flush turn-time calls、Evolution evidence/review/ledger/API/events、Goal turn lifecycle/follow-up scheduling、Workflow execution/validation/runner、RLM execution loop、Hook executor、Cycle/Seam、Automation shutdown ownership、Runtime/API event surface versioning。既有遗留：`build_system_prompt(subagent_mandate=True)` 引用缺失的 `deepseek_tui.engine.subagent_intent`；Evolution procedural skill 代码依赖 `yaml`，`pyproject.toml` 未声明 `PyYAML`；`knowledge_tools.py` 有既有 E501。详见 `docs/CAPABILITY_MODULE_REFACTOR_PROGRESS.md`。 | + `pytest tests/host tests/goal tests/memory tests/evolution tests/test_automation_manager.py tests/test_session_activity_integration.py tests/app_server/test_workflow_cancel_finalize.py tests/contract/test_workflow_progress_sse.py -q`：241 passed, 3 skipped, 1 warning（with pyyaml）；SubAgent wiring focused：14 passed；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.engine-wiring-adapters | — | **Engine wiring adapter batch（2026-06-11）**：新增 `capabilities/hooks.py`、`capabilities/cycle.py`、`capabilities/post_turn.py`；Hook dispatcher/lifecycle executor/legacy `metadata["hook_executor"]` binding、CycleConfig/optional SeamManager/cycle session id/start timestamp、PostTurn Memory+Evolution pipeline assembly 和 PostTurnOrchestrator start/stop helper 均从 `Engine` 内联逻辑迁到窄 capability adapters。保持不变：Hook lifecycle 执行、Cycle/Seam turn-time archive/summary、PostTurn `after_turn`/`flush_before_loss`/`on_main_tool_called` 调用路径、pipeline 顺序、事件/API surface。剩余高耦合区收敛为 Memory turn-time recall/capture/flush、Evolution evidence/review/ledger/API/events、Goal turn lifecycle/follow-up、Workflow/RLM 执行 loop、Automation shutdown ownership、Runtime/API versioned extension events。 | + focused wiring：21 passed；综合：248 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.service-first-expanded | — | **ServiceRegistry adoption expansion（2026-06-11）**：Hook executor 现在注册 typed `HookExecutor` 与 named `hook_executor` service，同时继续写 legacy `metadata["hook_executor"]`；SubAgentManager 现在除 typed service 外也注册 named `subagent_manager`。工具 lookup 扩展为 service-first：`task_tools`/`todo_tools` 的 `TaskManager`、`shell_tools` 的 lifecycle `HookExecutor`、`subagent_tools` 的 `SubAgentManager` 均按 context 字段 → typed service → named service → legacy metadata 顺序读取。保留动态 metadata：`workflow_*`、`rlm_progress_cb`、`subagent_runtime`、`parent_session_messages`、shell process stores、todo store、active task id 等仍是 per-call/per-turn 状态，不提升为长生命周期 service。 | + focused service：28 passed；综合：267 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.runtime-shutdown-adapters | — | **ToolRuntime shutdown adapter batch（2026-06-11）**：将已迁能力的关闭细节从 `ToolRuntime.shutdown()` 移入各自 capability adapter：Automation scheduler cancel/wait/cancel fallback → `stop_automation_runtime()`；SubAgent mailbox close + owned manager shutdown → `shutdown_subagent_runtime()`；TaskManager/McpManager/LspManager 关闭分别迁到 `shutdown_task_manager()`、`shutdown_mcp_manager()`、`shutdown_lsp_manager()`。保持不变：`ToolRuntime` 仍负责关闭顺序编排（Automation → SubAgent → Task → MCP → LSP）和 owns_* 语义；暂不调用 `context.services.shutdown()`，避免与现有 runtime ownership 双重关闭。 | + focused shutdown：29 passed；综合：272 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.memory-turn-helpers | — | **Memory turn-time helper migration（2026-06-11）**：将 Memory 专属 turn-time helper 从 `Engine` 移入 `capabilities/memory.py`：trivial recall skip、memory thread id resolution、`memory.md` enablement、capture message serialization、tool-call detection、TurnEvidence/flush evidence construction、smart-memory recall invocation、PostTurn disabled 时的 direct capture fallback。保持不变：Engine 仍负责这些 helper 的调用时机（turn start / final sync / compaction flush），PostTurn pipeline 和 Evolution evidence metadata surface 不改。 | + focused memory：147 passed, 3 skipped；综合：278 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.evolution-goal-helpers | — | **Evolution evidence + Goal lifecycle helper migration（2026-06-11）**：`capabilities/evolution.py::publish_turn_evidence()` 接管 `TURN_EVIDENCE_KEY` / `TURN_EVIDENCE_FACTORY_KEY` live/final metadata 与 `EvolutionPipeline.note_active_turn()`；`capabilities/goal.py` 接管 local thread rebinding、goal-mode hint、follow-up stale validation、turn start/fail/finish、pending steer extraction、follow-up dispatch eligibility。保持不变：Engine 仍决定调用时机和事件发送；Evolution review scheduling/ledger/API/events/tool response shaping 不改；Goal status surfaces/TUI-server rebinding 不改。 | + evolution focused：58 passed；goal+evolution focused：104 passed, 1 warning；综合：282 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.mcp-workflow-runtime-helpers | — | **MCP catalog + Workflow hint + Task bridge helper migration（2026-06-11）**：`capabilities/tasks.py::attach_task_mcp_bridge()` 接管 TaskManager `_shared_mcp_manager` executor reuse wiring；`capabilities/mcp.py` 新增 `mcp_manager_from_runtime_or_context()` 和 `build_mcp_augmented_tool_catalog()`，接管 runtime/context service lookup、warm cache、cold background discovery deferral、MCP/native catalog merge 与 tool profile filtering；`capabilities/workflow.py::workflow_mode_hint()` 接管 workflow mode turn hint。保持不变：MCP tool dispatch/API routes、Workflow execution/validation/runner/progress schema 不改。 | + focused MCP/workflow：27 passed；综合：287 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.cycle-seam-turn-helpers | — | **Cycle/Seam turn-time helper migration（2026-06-11）**：`capabilities/cycle.py` 新增 `apply_layered_context_checkpoint()` 与 `advance_cycle_if_needed()`，接管 Seam token 阈值判断、highest-level/recompact/produce_soft_seam 分支、working-set pinned indices、soft seam append、Cycle token 阈值判断、archive_cycle、保留最近 8 条消息与 cycle 状态更新。保持不变：Engine 仍决定调用时机；Cycle/Seam 默认 opt-in；archive format、logger 行为和 failure best-effort 语义不改。 | + focused Cycle/Seam：18 passed；综合：289 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.mcp-dispatch-helper | — | **MCP external dispatch helper migration（2026-06-11）**：`capabilities/mcp.py` 新增 `is_external_mcp_tool_call()` 与 `execute_mcp_tool()`，接管 external MCP tool 判定、missing manager 错误、调用 `execute_external_mcp_tool()` 与 result shaping；Engine 仍负责 approval gate 和事件时机。保持不变：MCP API routes/surface 与 MCP manager implementation 不改。 | + focused MCP dispatch：30 passed；综合：305 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed；`tests/test_mcp_engine_integration.py` 存在既有 ruff import/line-length 问题，未纳入 scoped ruff 以避免无关格式 churn |
| capability-modules.phase1-7.workflow-execution-helper | — | **Workflow tool execution helper migration（2026-06-11）**：`capabilities/workflow.py` 新增 `resolve_workflow_spec()` 与 `execute_workflow_tool()`，接管 WorkflowTool 的 script+spec merge、validation error shaping、SubAgentManager/loop runtime 检查、DeepSeekAgentRunner 组装、progress/status callbacks、timeout/cancel/spawned-agent cancel、result rendering 与 metadata payload；`tools/workflow_tool.py` 缩减为工具声明和 capability helper 调用。保持不变：Workflow runtime package、progress event schema、server/TUI rendering 不改。 | + focused Workflow：34 passed；综合：328 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.rlm-evolution-api-helpers | — | **RLM execution + Evolution API helper migration（2026-06-11）**：`capabilities/rlm.py` 新增 `execute_rlm_tool()`，接管 RLM input validation、file/inline loading、child model pinning、progress callback lookup、`run_rlm_turn()` 调用、error shaping、report/trace rendering 与 metadata payload；`tools/rlm/tool.py` 缩减为工具声明和 capability helper 调用。`capabilities/evolution.py` 新增 `evolution_record_to_dict()` 与 `evolution_ledger_for_thread()`，Workbench Evolution approval routes 转调 capability helper。保持不变：RLM turn loop internals、RLM progress event schema、Evolution URLs/status codes/response shape、review scheduling、ledger mutation behavior 与 events 不改。 | + focused RLM/Evolution：70 passed, 1 warning；综合：351 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.goal-mcp-surface-helpers | — | **Goal status + MCP runtime route helper migration（2026-06-11）**：`capabilities/goal.py` 新增 `bind_goal_runtime_thread()`、`goal_status_payload()`、`take_valid_goal_follow_up()`，接管 runtime thread rebind/on-change binding、GoalChip `goal.status` payload 构造和 pending follow-up take+validate；`RuntimeThreadManager` 仍决定 emit 时机和 hidden follow-up turn scheduling。`capabilities/mcp.py` 新增 `mcp_startup_response()` / `mcp_preload_status_response()`，`/v1/mcp/startup` 与 `/v1/mcp/preload-status` routes 转调 helper。保持不变：Goal SSE event name/payload shape、follow-up request fields、MCP URL/status/response shape、AppRuntime MCP startup/preload implementation 不改。 | + focused Goal/MCP：32 passed；综合：368 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.memory-evolution-tool-helpers | — | **Memory turn-start + Evolution main-tool response helper migration（2026-06-11）**：`capabilities/memory.py` 新增 `MemoryTurnContext` / `prepare_memory_turn_context()`，接管 turn-start memory search budget reset、thread id resolution、smart recall invocation 和 user-message recall injection wrapping；`Engine` 保留 turn lifecycle 时机。`capabilities/evolution.py` 新增 `evolution_decision_from_record_status()` / `build_main_tool_evolution_response()`，`memory_curate` 与 `skill_manage` 转调 capability helper。保持不变：Memory recall/capture payload、prompt injection tag、Evolution tool result JSON shape、review scheduling、ledger mutation 与 event surface 不改。 | + focused Memory/Evolution：23 passed, 1 warning；综合：370 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.postturn-dispatch-helpers | — | **PostTurn dispatch helper migration（2026-06-11）**：`capabilities/post_turn.py` 新增 `run_post_turn_after_turn()` 与 `flush_post_turn_before_loss()`，接管 PostTurn `after_turn` dispatch、PostTurn disabled 时 direct smart-memory capture fallback、compaction 前 `flush_before_loss` dispatch；`Engine` 仍决定触发时机和 evidence 构造。保持不变：PostTurn pipeline order、orchestrator behavior、Memory capture payload、flush-before-loss payload 和 compaction behavior 不改。 | + focused PostTurn/Memory/Evolution：34 passed, 1 warning；综合：372 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.goal-mcp-followup-route-helpers | — | **Goal follow-up + MCP legacy route helper migration（2026-06-11）**：`capabilities/goal.py` 新增 `GoalFollowUpStartPayload`、`goal_follow_up_is_stale()`、`build_goal_follow_up_start_payload()`，接管 hidden goal follow-up stale validation 和 hidden continuation request payload 构造；`RuntimeThreadManager` 仍决定触发时机并调用 `start_turn()`。`capabilities/mcp.py` 新增 MCP runtime-response helpers，legacy app-server `/mcp/startup`、`/apps/mcp/servers`、`/apps/mcp/tools` 和 stdio MCP startup 转调 helper。保持不变：Goal hidden turn request fields、Goal stale error、MCP URLs/status/response shape、AppRuntime MCP implementation 不改。 | + focused Goal/MCP：34 passed；综合：374 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.host-contribution-registries | — | **Host lifecycle/surface/contribution registry foundation（2026-06-11）**：新增 `host/lifecycle.py`（before-turn、turn-complete、turn-failure、before-tool、after-tool observer phases）、`host/surfaces.py`（runtime route/event presenter contribution registry）、`host/contributions.py`（services、ToolPacks、PromptContributors、Lifecycle、PostTurn、Surfaces 聚合容器）；`CapabilityModule.contribute()` 类型收口到 `Contributions`，并避免 host/tools 运行时 import cycle。保持不变：这些 registry 暂不接业务执行路径，不改变 Engine events、API routes、ToolRuntime、prompt 或 tool dispatch 行为。 | + focused host registry：17 passed；综合：383 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.memory-before-turn-observer | — | **Memory before-turn lifecycle observer migration（2026-06-11）**：`capabilities/memory.py` 新增 `MemoryBeforeTurnObserver`、`memory_before_turn_observer()` 与 `MEMORY_TURN_CONTEXT_DECORATION`；Engine turn-start 通过 `LifecycleRegistry.before_user_turn()` 调用 Memory observer，并从 decorations 读取 `MemoryTurnContext`。保持不变：Engine 仍决定 turn-start 触发时机，memory search budget reset、thread id resolution、smart recall、user-message injection wrapping、prompt recall payload 与 capture/flush 行为不改。 | + focused Memory observer：103 passed, 3 skipped；综合：384 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.phase1-7.postturn-after-tool-observer | — | **PostTurn/Evolution after-tool lifecycle observer migration（2026-06-11）**：`capabilities/post_turn.py` 新增 `PostTurnToolObserver`、`post_turn_tool_observer()` 与 `notify_post_turn_main_tool_called()`；Engine tool-result success path 通过 `LifecycleRegistry.after_tool()` 触发 PostTurn main-tool notification，从而保留 EvolutionPipeline `on_main_tool_called()` 对 `memory_curate` / `skill_manage` 的 scheduler reset 行为。保持不变：工具执行/approval/ToolResultEvent/working-set/LSP hook 时序不改，Evolution review scheduling、ledger mutation 和 event surface 不改。 | + focused PostTurn/Evolution observer：24 passed, 1 warning；综合：385 passed, 3 skipped, 1 warning（with pyyaml）；scoped `ruff check` passed；compileall passed |
| capability-modules.remaining-handoff | — | **Remaining work handoff（2026-06-11）**：新增 `docs/CAPABILITY_MODULE_REFACTOR_REMAINING.md`，按 `CAPABILITY_MODULE_REFACTOR_PLAN.md` 拆分剩余工作：assembler contribution collection、Engine-owned lifecycle registry、Goal lifecycle observer、RuntimeThreadManager Goal service adapter、LSP observer、Memory capture/flush trigger timing、Evolution review/ledger events、Workflow/RLM typed execution context、surface registry integration、MCP manager internals、compatibility debt removal，并列出每项目标、文件、步骤、验收与风险。 | docs only |
...
### Stage 8（6–8 周）：DeepSeek Workbench（桌面 GUI） — **Stage 8.7 已完成（2026-05-26）**

> **完整规划（文件清单 / 架构 / Sprint / 契约）见 [`docs/WORKBENCH_HANDOVER.md`](./WORKBENCH_HANDOVER.md)。** 本节只列摘要。

**当前状态**：`runtime_api` + Electron 托管 + auth 链 + 聊天/审批/会话统一已通过；**门禁** `pytest tests/contract -q`（约 **69**）；TUI 主路径 smoke：`tests/test_tui_smoke.py`。待办见 [`docs/WORKBENCH_BACKLOG.md`](./WORKBENCH_BACKLOG.md)（**不做**：electron-builder 打包、Claw GUI 暴露）。
...
**与当前 app_server 差别**：约 40%–60% 新建/重写 HTTP 面；Engine 基本不动；历史 `tests/parity/phase_*` 已退役，日常以 `tests/contract` + `tests/test_tui_smoke.py` 为门禁。
...
| ✅ memory.l1_live_api_test | P1.smart | `tests/memory/test_l1_extraction_live.py`：直接 L1 提取 + capture/flush 调度链；`conftest.py`（`DEEPSEEK_SKIP_KEYRING`）；无 key 时 skip | HANDOVER 原则 A | `DEEPSEEK_SKIP_KEYRING=1 DEEPSEEK_API_KEY=… pytest … -m live -v` |
| ✅ memory.p1_native_wired | P1.smart | **2026-05-29**：Native L0+L1+FTS + L2/L3 + hybrid RRF；`Engine` recall/capture（`TurnOutcomeStatus` 门控）/ flush；Workbench `memory_mode`；TUI resume `memory_thread_id`+`memory_mode`；无 Sidecar。默认 `[memory.smart] enabled=false`。`tests/memory/` + contract；live：`-m live` | — | 手验 C–F 见 `MEMORY_INTEGRATION_BACKLOG.md` |

...
| ⚠️ batch-2#F3.events SSE 30-tick window | pre-realapi-batch-2 | `/threads/{id}/events/stream` 是 30 tick × 100ms = 3 秒窗口，不是 Rust 的真实 long-poll subscribe | Python 没有 `subscribe_events` channel | 在 `RuntimeThreadStore` 加 `asyncio.Queue` 订阅器：每个 `_emit_event` 把事件 fan-out 到所有活跃订阅；route 把订阅 queue 接到 SSE generator 直到客户端断开 |

...
LONG_SESSION_REMINDER = (
    ...
    "- Incorporate the latest request; status questions and added constraints "
    "do not cancel unfinished work. Honor explicit stops and replacements.\n"
...
CHECKLIST_GATE_REMINDER = (
    ...
    "- Removed from scope by the user: mark it `cancelled` and say why.\n"
    "- Blocked on necessary input, approval, or an unavailable capability: "
    "finish independent work, then report the blocker, remaining scope, and "
    "what would allow continuation. Do not treat a partial implementation "
    "or recoverable failure as a reason to cancel.\n"
    "Never mark an item completed with unmet required acceptance criteria. "
    "Report pre-existing or unrelated test failures separately. Items still "
    "open when the turn ends are closed as cancelled by the runtime; for "
    "blocked work this is bookkeeping, not user cancellation or completion. "
    "Preserve the remaining work in your reply and restore its tracking when "
    "the task resumes. If you already "
...
COMPACT_CONSUMER_HINT = (
    "## After Compaction\n\n"
    "When earlier turns are compacted, a handoff note you wrote to yourself "
    "appears in `<archived_context>`. It always carries a `Goal` and a "
    "`Next step`; the rest of its shape follows whatever the session needed "
    "(typically Constraints, Progress, Unknowns, Key Decisions). Treat that "
    "block plus the recent verbatim messages as your continuation context — do "
    "not ask the user to restate work that is already covered there.\n\n"
    "Read it as notes, not as proof. Where it records a step as done, a test as "
    "passing, or a fix as working, verify that yourself before building on it; "
    "an `Unknowns` section names what the earlier turns never actually "
    "established, and those are gaps to go check rather than facts to "
    "assume.\n\n"
    ...
    "verbatim; the note is a paraphrase. **Where the two disagree, the "
    "verbatim requests win.** A constraint listed there still binds you even "
    "if the note omits it — the note losing it is the expected failure, "
    "which is why the list exists. Incorporate the last entry into the active "
    "task: a status question or added constraint does not cancel unfinished "
    "work. Honor explicit stops and replacements; continue remaining "
    "authorized work without redoing completed steps."
)

...
export function RuntimeDiagnosticsDialog({
  ...
  const workspaceRuntime = useMemo(() => {
    const body = diagnostics?.runtime.workspaceStatus?.body
    if (!body) return null
    try {
      const parsed = JSON.parse(body) as {
        runtime_api?: { service?: string; python_version?: string; mode?: string }
      }
      return parsed.runtime_api ?? null
    } catch {
      return null
    }
              ...
              <section className="flex min-w-0 flex-col gap-4">
                ...
                <div className="rounded-2xl border border-ds-border bg-ds-card p-3">
                  ...
                  <dl className="mt-3 grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-2 text-[12px] leading-5">
                    ...
                    <dd className="min-w-0 break-words text-ds-ink">
                      {workspaceRuntime
                        ? `${workspaceRuntime.service ?? 'deepseek-runtime-api'} (${workspaceRuntime.mode ?? 'http'})`
                        : responseSummary(diagnostics?.runtime.workspaceStatus ?? null) || '-'}
                    </dd>
                    <dt className="text-ds-muted">Python</dt>
                    <dd className="min-w-0 break-words text-ds-ink">{workspaceRuntime?.python_version ?? '-'}</dd>

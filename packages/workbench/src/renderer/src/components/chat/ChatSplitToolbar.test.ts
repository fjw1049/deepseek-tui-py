...
it('exposes the selected arrangement, disables the seventh pane without a pane count or exit control', async () => {
  ...
  expect(trigger.textContent).toBe('splitGrid')
  expect(document.querySelector('[role="dialog"]')).toBeNull()
  await act(async () => trigger.click())
  const panel = document.querySelector('[role="dialog"]')!
  const options = [...panel.querySelectorAll<HTMLButtonElement>('button')]
  expect(options.map(button => button.textContent)).toEqual(['splitCompactView', 'splitGrid', 'splitHorizontal', 'splitVertical'])
  expect(options[1].getAttribute('aria-pressed')).toBe('true')
  await act(async () => options[3].click())
  expect(onArrange).toHaveBeenCalledWith('vertical')
  await act(async () => root.render(createElement(ChatSplitToolbar, {
    project: '/repo', onFocus: vi.fn(),
    layout: { panes: ['a', 'b', 'c', 'd', 'e', 'f'].map(id => ({ id, threadId: id })), focused: 'a', x: .5, y: .5, arrangement: 'vertical' },
    onArrange, onAdd
  })))
  expect(trigger.textContent).toBe('splitVertical')

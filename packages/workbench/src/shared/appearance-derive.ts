...
export function buildChromeThemeCssVars(
  ...
  if (theme.uiFont) {
    ...
    vars['--font-display'] = withUiFallback(theme.uiFont)
  }
  if (theme.codeFont) {
    vars['--font-code'] = withMonoFallback(theme.codeFont)
  }
...
function withUiFallback(family: string): string {
  ...
  return generic ? family : `${family}, 'SF Pro Text', 'PingFang SC', 'Noto Sans SC', sans-serif`
}
